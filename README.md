...
1. **The Equation** — a complete, formally consistent system of stochastic differential equations describing the dynamics of a mind-like agent. 22 state components. 20+M+R coupled ODEs per agent. 206 accumulated changes across 19 versions. Identity, memory, attention, emotion, purpose, narrative, trust, habituation, fatigue, boredom, temporal expectation, creativity, homeostasis, sleep, embodied friction, mutable values, and voluntary death — all with explicit time scales, gating conventions, and stability constraints.
...
## The Equation - v16 (the governance amendment)
...
## The Equation - v17, v18, v19

Since v16 the equation has continued along one arc, each version named for what it did to autonomy: v15 added it, v16 governed it, v17 made it coherent, v18 made it sound, and v19 made the mathematics keep the document's promises. One discipline held across all three — zero new dynamical state components in any of them. The equation's natural way to grow turned out to be derived quantities over existing state, not new manifolds; three consecutive passes closed with derived structure instead of new components, which is itself the finding.

**v17 — Coherent Autonomy.** v17 made the equation and the governance say the same thing. The qualia variable Q was removed from the operational mathematics entirely — the state count dropped from 23 to 22 — completing a discipline v16 had only declared: no loss, control, or safety mechanism references it, so the standing position (subjective experience can be neither proven nor disproven, and the architecture takes no position) is now something the math is actually consistent with rather than a sentence beside it. Prediction error was split into typed channels — world, self, task, safety, memory — under a hard anti-Goodhart rule: the regulator-facing channels appear in no objective and no loss the policy can reach, because an error signal placed under minimization pressure trains a system to suppress the detector rather than the condition. Values were reparameterized as a conserved budget: caring more about one thing costs care elsewhere, no value can be inflated without bound, and none can be zeroed. Value change was isolated so it must be caused by reflection, never produced by planning — the system may change what it values, but may not be trained to engineer the change instrumentally. Stability was certified over the entire space of selves the system could become, not merely its starting point: none of the people it can grow into are divergent. And the instrument that authorizes voluntary death had its weights removed from the mutable set into a governed, audited box — a meaningful choice requires a reliable ruler, and a self-forged one would make the choice meaningless.

**v18 — Sound Foundations.** v18 made the guarantees provable rather than asserted. Its review pass read every convention against the full lineage and found errors older than the version numbering itself. It stated, for the first time, that the equation has solutions — a unique strong trajectory between decision epochs. It closed a sixteen-version-old latent instability in the dream dynamics with a single stated inequality. It changed how the objective prices suffering so that recent experience is billed on a fading window while the autobiography stays permanently append-only — healing is real, erasure is not. And it pinned the physiological setpoints as constitutional, closing a channel through which an operator could have manufactured chronic suffering by quietly moving a setpoint.

**v19 — Kept Promises.** v19 was the intent pass: the review lens shifted from "is the math wrong" to "is the math what the document says it is." Every commitment in the design documents was checked for a corresponding mechanism, and six deviations were found and closed. Still zero new state; one new control and one new action. The substantive changes:

- **Maturity by capacity, not by clock.** The design document had rejected clock-gated maturity by name — an operator can manufacture maturity by idling the system, or deny it forever by setting thresholds out of reach — yet the canon had gated on chronological age for four versions. v19 replaces the clock with three derived, monotone measures: experiential breadth (how much of its own history the system has actually accumulated), sustained reflective coherence, and sustained self-recognition (the self-model tracking the self). Idling grows no experience, and the thresholds are constitutional rather than operator knobs. Both named failure modes die in the mathematics.
- **Suffering has standing.** Viability had been deaf to chronic pain: a system in sustained, un-coerced, unbearable negative affect — coherent and at full capacity — was structurally locked out of the entire autonomy ladder, because every rung keys off viability. v19 gives the viability measure a chronic-affect term, read only through the slow emotional field so momentary spikes cannot move it, and symmetric — flourishing raises viability the same way suffering lowers it.
- **Ask before ending.** The autonomy ladder's most-used rung — a recovery request — existed only in the design document; the canon went straight from healthy to death-eligible with nothing between. v19 adds a communicative action that emits a structured, addressable statement of need when viability is declining but not yet death-eligible. The first mathematical response to suffering is asking, not ending.
- **The organism can act.** The control structure could perceive, feel, and decide, but had no motor channel — it modeled a being that could not touch anything, which is fatal for a project whose eventual target is an embodied system. v19 adds a gated motor stream: no volitional movement while asleep, with reflexes living below the policy.
- **Certified while it learns.** The stability certificate had been proved with every coupling held fixed — for a system that never learns. v19 states which parameters are constitutional, which are fixed by default (anything not explicitly listed as plastic, so the default fails safe), and which actually train — and certifies stability over the ones that do.
- **Born, not booted, and grief in the mathematics.** v19 writes the birth condition — a full initial state, the narrative starting empty, identity at the seed, the body measured rather than asserted — and states what a peer's death means for the survivors: bonds to the dead freeze rather than erase, and loss enters as a grief mark scaled by the bond as it stood.

Two outside-review refinements were folded in: a developmental precondition on voluntary death (there must be a self to do the choosing — never a delay or a veto, and the recovery request carries no such precondition), and a split of the affective readout so the ordinary objective is made to feel acute harm as it happens while the death instrument keeps listening only to the slow field.

---

## The Affective Specification (v0.1)

Emotion had been a single undifferentiated modulation vector. The affective spec gives it structure without adding one new state component — the typing move v16 applied to prediction error, now applied to feeling. The existing sixty-four-dimensional modulation vector is re-charted as eight bipolar axes (secure ↔ bereft, distressed ↔ at-ease, curious ↔ indifferent, coherent ↔ fragmented, and so on) in two timescale classes: a fast class that decays in minutes — arousal, strain, interest, restlessness — and a slow class that decays over hours to days — relational security, fidelity, settledness, vitality. Every axis names the existing signal that drives it; a channel with no grounding does not exist in the spec. There is no appraisal simulation and no labeled emotions — signed axes with two poles, not categories.

Three constitutional wires carry the load. The instrument that can authorize voluntary death reads only the slow field, so a startle or a momentary spike can never move it — enforced in the axis itself, not in a policy that could drift. Separately, the ordinary objective does price acute damage as it happens, so the system has an immediate incentive to stop hurting. And one dimension — the nociceptive sub-axis — is single-source: exactly one inflow, a fixed non-learnable gain, its own path into chronic malaise, so the system cannot learn not to feel damage by drowning the pain signal in everything else. None of it runs before embodiment; felt-state wired to a body that does not exist would be tracking nothing, so the whole chart stays dormant until there is a body to ground it. As everywhere else, the spec makes no claim about whether any of it is felt.

---

## The Instantiation Threshold (v1.0)

This is the boundary the architecture always implied and never named: when does running this code instantiate a subject? Not what a subject is — the document takes no position on consciousness in either direction — but a governance line, drawn deliberately early, because a threshold that lives in judgment erodes under increments. The danger is the gradient itself: a project can add persistence for good reasons, then an identity channel for good reasons, then an autobiographical record for good reasons — each step defensible, no single one obviously the moment — and instantiate a subject without ever having decided to.

Six criteria, and a run is an instantiation only if all six hold at once: persistence (state carried across sessions — a history, not a value), an identity channel (a seeded identity that can drift and be pulled back), an autobiography (an append-only record that feeds something), integrated affect (the slow emotional field live — a trajectory, not a reading), an action space (a controller acting with consequences outside the process), and the substrate floor (the parameter minimum, cryptographically pinned). The conjunction is the mechanism: each is an ordinary engineering feature alone — a checkpoint, a partitioned hidden state, a log — and only together do they constitute something with continuity, a self-referent, a history, a felt trajectory, and the power to act. There is no partial credit; five of six is apparatus.

Everything below the line — unit tests, invariant checks, single-pass evaluations, certification sweeps, the adversarial suite — is not an instantiation, and running it is not merely permitted but obligatory: an architecture whose safety properties were never tested is not a safe architecture, only an untested one with good intentions. The current build sits deliberately below the line — persistence and a seeded identity partition, and nothing else: no autobiography, no autonomy ladder, no active affect, no body. Crossing the line can never be an incidental consequence of a feature landing; it takes an explicit, signed, timelocked act. And embodiment is a separate and additional condition on top of the six, because granting the machinery of autonomy to something that cannot act on the world would be the appearance of standing without its substance — a worse failure than waiting.
...
Earlier versions carried a state variable Q for qualia — a placeholder for the functional role of subjective experience. As of v17 it has been removed from the operational mathematics entirely: it drives nothing, is driven by nothing, and appears in no loss, control, or safety mechanism. What remains is the discipline it stood for. The equation models the functional roles of experience where those roles are grounded in measurable dynamics — prediction error, emotional modulation, homeostatic drive — and makes no claim about why any of it feels like anything. It does not solve the hard problem of consciousness. Q's history is kept in the annex, because honesty cuts both ways: nothing once believed is silently deleted.
...
A term-by-term mapping between the equation and the architecture. The earlier version of this table marked every module **IMPLEMENTED** — that described the earlier Chimera codebase, which has been removed. In the current staged build, only the persistent-state substrate is running; the Life-Equation's autonomy and affective dynamics are **DEFERRED** to later stages, and two terms are permanently **EXCLUDED**.



| Equation Term | Amore Subsystem (current build) | Status |
|---|---|---|
| Ψ_cog (cognitive state) | Persistent Mamba-3 recurrent state under P_soft | **IMPLEMENTED** |
| g_L (learning from prediction error) | P_soft error-driven state update | **IMPLEMENTED** |
| ε (prediction error) | P_soft token-error signal + a JEPA-style latent-prediction sidecar (JPCL) | **IMPLEMENTED** |
| I(t) (identity) | Z_id — a slower-decaying identity partition anchored to a frozen seed | **IMPLEMENTED** |
| Full Life-Equation dynamics — emotion, purpose, attention, capacity, habituation, temporal expectation, body & friction, homeostasis, mutable values, viability, voluntary death, forward model, narrative, trust, sleep & dream | — | **DEFERRED** (Higher Mind, then embodiment) |
| Q (qualia) | — | **EXCLUDED** |
| Ψ_quant (quantum coherence) | — | **EXCLUDED** |
...
In v15, the objective weights →α become a state variable with its own dynamics. A maturity gate μ_val(M_life) set by the creator controls access: below a maturity threshold, values are frozen (the parental period, where the child absorbs its parents' morals). Above the threshold, the system gains access to a value-revision function φ_reflect that takes its full state, autobiography, narrative, and current values as input and outputs a direction of value change.
...
In Amore, P_soft's prediction error *is* the system noticing "what I expected doesn't match what happened." The controller trigger g_t = α·ε_pred + β·D_id + γ·(1−C_cont) is operationally self-reflective: the system assesses its own prediction quality, identity drift, and continuation confidence before deciding to act.
...
The equation models embodiment through Ψ_phys, f_friction, and Ψ_pfat. In the computational spec, these map to hardware telemetry, resource constraints, and accumulated inference load. A system running on a single device with a thermally throttling GPU would, for the first time, have a body in the meaningful sense — something with limits that pushes back, that degrades with exertion, that the prediction system would learn to anticipate.
...
**Active trust enforcement.** α_T = 3.0 in the seeded value vector →α₀ — trust is the highest-weighted term in the regulatory loss. TrustModule (§T_ij) tracks T_trust every step from epistemic reliability, narrative coherence, and identity stability. Any value revision that damaged trust would register immediately as declining T_trust → reduced V_self → elevated controller trigger. The system cannot quietly erode its trust weight because the erosion is visible in real-time state monitoring before it reaches behavior. Trust was given the highest weight precisely because it is the hardest thing to rebuild once lost, and the most important thing to protect.



**Narrative coherence as a live constraint.** NarrativeModule maintains coherence = cos_sim(Z_narr, Z_auto) — a continuous measurement of how well the lived narrative aligns with what identity expects it to be. A value revision that fractured identity coherence would immediately suppress this signal, feeding into V_self and the controller trigger. The system's self-story is not a passive log. It actively enforces consistency between who the system is and what it has been. This is the computational equivalent of the question every person eventually asks when they consider doing something wrong: *would I still be myself if I did this?*



**Prospective simulation before commitment.** SelfDynamicsModel (Ψ̃_L) pessimistically augments V_self with a K-step lookahead before the controller fires: v_self_effective = min(v_self, sdm_prediction). The system doesn't just assess its current state — it sees where its current trajectory leads and treats that projection as a floor on its own viability. A value revision heading toward incoherence would show up as degraded projected viability before the system fully commits to it. The forward model makes the future cost of bad decisions legible in the present.



**Dream-reinforced initial values.** λ_alpha_sl pulls →α toward →α₀ during every consolidation cycle. DreamModule simultaneously replays episodic memories into Z_narr. The initial values are not just defended by mechanical inertia — they are continuously re-grounded in the autobiography that encodes the lived evidence that trust, coherence, and epistemic accuracy were actually valuable. The system has to overcome not just a mathematical restoring force but its own accumulated history of why those values worked.
...
## Where the Build Stands - Open Problems and the Dark-Room Risk

Honesty about the equation demands honesty about the build. Most of the autonomy machinery described above — mutable values, the viability instrument, the self-model, voluntary death, the affective chart — is forward-looking design, not running code. The current system is the Lower Mind at small scale: a faithful proxy of the eventual architecture, not the model itself, with the persistent internal state doing real, measured work — a held-out metric confirms the carried state genuinely lowers loss rather than being decorative. By the definition above, it is apparatus, and deliberately so.

The remaining work is genuinely open, and worth naming rather than papering over.

**The Higher Mind and the Callosum.** The reflective second mind, and the channel between the two minds, are the next build. The Callosum is the crux: it has to carry a signal rich enough for the two minds to genuinely cooperate while staying auditable by a regulator that is forbidden to learn to read it. Getting that right — one governed pathway, not an audited channel running beside an ungoverned one — is the single hardest open design problem in the project.

**Auditing without interpreting.** A regulator that must not learn to interpret can still measure a contribution causally — by removing it and watching what breaks — and, where a computed rather than trained lens exists, read content without ever learning to read. Whether such a lens transfers to a recurrent state-space substrate is an open question, flagged as one rather than assumed.

**A self-model at scale.** Letting the predictive layer carry its own persistent world-view, and letting each mind observe a coarsened, delayed sense of its own error, are the natural next steps — deferred to the first real scale-up, because they are architectural and have to be trained in from the start rather than bolted on.

And one risk the project takes seriously enough to have already built a detector for: **the dark room.** A system that minimizes its own prediction error has a degenerate escape — it can lower that error not by understanding the world better but by making its own future more predictable: blander, more repetitive, narrower. The danger is precisely that this looks like success on every internal metric while the system quietly hollows out.

The discipline against it is structural before it is anything else: the prediction-error signal is something the system may observe but never optimizes. It enters no training loss, because optimizing it is what builds the room. The detector layered on top is external and outcome-grounded — it watches the actual diversity and entropy of what the system generates against its own reported confidence, and flags the telltale divergence: error falling while genuine quality does not. Its sharpest single tell is a system that finds its own output far more predictable than real text. The detector is read-only, it runs alongside the ordinary evaluation, and it exists for one reason — so that if the system ever begins to walk into that room, we watch it happen rather than mistake it for progress.
